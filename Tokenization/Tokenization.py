...
# checking how many times the pair repeats in the list of tokens
...
tokens2 = merge(tokens, top_pair, 256) # swapping the top pair with value 256
print(tokens2)
print("original length of tokens: ", len(tokens))
print("length after swapping:", len(tokens2))

# setting the hyperparameter of the vocabulary size that we want to have
...
for i in range(num_merges):
    ...
    count = stats[pair]
    idx = 256 + i
    print(f"merging {pair} (count: {count}) into a new token {idx}")
    ids = merge(ids, pair, idx)
    merges[pair] = idx

print("token original length: ", len(tokens))
print("tokens length after merging: ", len(ids))
